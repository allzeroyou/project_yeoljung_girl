...
keras.layers.Conv2D(64, kernel_size=(3, 3), activation='relu',input_shape=(28,28,1)),  # 필터의 수 32개, 필터 크기 3 X 3, 활성화 함수 'relu', 입력 이미지 크기 28 * 28, 입력 이미지 채널 1( 흑백일 경우 채널1, 컬러일 경우 채널 3)
keras.layers.MaxPooling2D(pool_size=(2, 2)),
keras.layers.Conv2D(64, kernel_size=(3, 3),  activation='relu'), #컨볼리션 레이어 : 필터 크기 3*3, 필터 수 64개, 활성화 함수 'relu'
keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2,2)), #맥스 풀링 레이어 : 풀 크기  2 * 2
keras.layers.Conv2D(128, kernel_size=(2, 2), activation='relu'), #컨볼리션 레이어 : 필터 크기 3*3, 필터 수 32개, 활성화 함수 'relu'
keras.layers.MaxPooling2D(pool_size=(2, 2)), #맥스 풀링 레이어 : 풀 크기  2 * 2
keras.layers.Flatten(), #플래튼 레이어
keras.layers.Dense(256, activation='relu', kernel_initializer='he_normal'), #댄스 레이어 : 출력 뉴런수 256개, 활성화 함수: 'relu'
keras.layers.Dense(10, activation='softmax', kernel_initializer='glorot_normal') #댄스 레이어: 출력 뉴런수 10개, 활성화 함수 'softmax'
    ]
)
...
# 모델 학습시키기
hist = model.fit(x_train, y_train, batch_size= 10, epochs=15)
